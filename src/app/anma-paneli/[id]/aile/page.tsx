...
const REL_LABELS: Record<string, string> = {
  ...
  daughter: 'Kızı',
  sibling: 'Kardeşi',
  gm_maternal: 'Anneannesi',
  gf_maternal: 'Dedesi (Anne T.)',
  gm_paternal: 'Babaannesi',
  gf_paternal: 'Dedesi (Baba T.)',
  uncle: 'Amcası/Dayısı',
  aunt: 'Halası/Teyzesi',
...
export default async function MemorialAilePage({ params, searchParams }: Props) {
  ...
  const pageUrl = `/anma-paneli/${id}/aile`
  const editMember = editId ? (members ?? []).find(m => m.id === editId) : null
  const parentCandidates = members ?? []

  const dInputCls =
    'w-full rounded-xl border border-white/15 bg-white/5 px-4 py-3 text-sm text-[#efe7d8] placeholder-white/30 outline-none focus:border-[#c7a76f]/60 focus:ring-1 focus:ring-[#c7a76f]/20'
  const dSelectCls =
    'w-full rounded-xl border border-white/15 bg-[#0d1412] px-4 py-3 text-sm text-[#efe7d8] outline-none focus:border-[#c7a76f]/60'
  const dLabelCls = 'mb-1.5 block text-xs font-semibold text-[#c7a76f]/80'

  return (
    <div className="min-h-screen bg-[#091712] text-[#efe7d8]">
      {/* Sticky header */}
      <div className="sticky top-0 z-20 border-b border-white/8 bg-[#091712]/95 px-5 py-4 backdrop-blur sm:px-8">
        <div className="mx-auto flex max-w-5xl items-center justify-between">
          <div className="flex items-center gap-2 text-sm">
            <Link
              href={`/anma-paneli/${id}`}
              className="text-[#c7a76f]/60 transition-colors hover:text-[#c7a76f]"
            >
              {vault.display_name}
            </Link>
          ...
          {!isLocked && (
            <Link
              href="?add=1"
              className="inline-flex items-center gap-1.5 rounded-xl bg-[#c7a76f] px-4 py-2 text-xs font-semibold text-[#091712] transition-colors hover:bg-[#d4b87c]"
            >
              + Üye Ekle
            </Link>
          )}
                  ...
                  {editMember ? (
                    <>
                      <div className="mb-5 flex items-center justify-between">
                        <div className="flex items-center gap-3 text-[#c7a76f]">
                          ...
                          <span className="text-xs uppercase tracking-[0.2em]">Düzenle</span>
                        </div>
                        <Link
                          href={pageUrl}
                          className="text-xs text-[#8f9f96] transition-colors hover:text-[#c7a76f]"
                        >
                          ← İptal
                        </Link>
                      </div>
                      <form
                        action={updateMemorialFamilyMemberAction.bind(null, editMember.id, id)}
                        className="space-y-3"
                      >
                        <div className="grid grid-cols-2 gap-3">
                          <div>
                            ...
                            <select
                              name="relationship"
                              required
                              className={dSelectCls}
                              defaultValue={editMember.relationship}
                            >
                          ...
                          <div>
                            <label className={dLabelCls}>
                              ...
                              <span className="font-normal text-white/30">
                                (opsiyonel — başka birinin yakını ise seçin)
                              </span>
                            </label>
                            <select
                              ...
                              className={dSelectCls}
                              defaultValue={
                                (editMember as Record<string, unknown>).parent_member_id as string ?? ''
                              }
                            >
                              <option value="">Profil sahibine bağlı</option>
                              {parentCandidates.map(p => (
                                <option key={p.id} value={p.id}>
                                  {REL_ICONS[p.relationship] ?? '👤'} {p.full_name}
                                </option>
                              ))}
                            </select>
                          </div>
                        ...
                        <div className="grid grid-cols-3 gap-3">
                          ...
                          <div>
                            ...
                            <select
                              name="is_alive"
                              defaultValue={editMember.is_alive ? 'true' : 'false'}
                              className={dSelectCls}
                            >
                        ...
                        <div>
                          ...
                          <input
                            type="url"
                            name="photo_url"
                            defaultValue={editMember.photo_url ?? ''}
                            placeholder="veya URL..."
                            className={`${dInputCls} mt-2`}
                          />
                        </div>
                        <div>
                    ...
                    <>
                      <div className="mb-5 flex items-center justify-between">
                        <div className="flex items-center gap-3 text-[#c7a76f]">
                          <span className="h-px w-6 bg-[#c7a76f]" />
                          <span className="text-xs uppercase tracking-[0.2em]">Yeni Üye Ekle</span>
                        </div>
                        <Link
                          href={pageUrl}
                          className="text-xs text-[#8f9f96] transition-colors hover:text-[#c7a76f]"
          ...
          {(members?.length ?? 0) === 0 ? (
            <div className="overflow-hidden rounded-2xl border border-white/10 bg-[#0d1412] shadow-2xl shadow-black/30">
              <div className="py-16 text-center">
                ...
                <p className="mb-2 font-serif text-xl text-white">Aile ağacı boş</p>
                <p className="mx-auto mb-5 max-w-xs text-sm text-[#8f9f96]">
                  Üye ekle butonuna tıklayarak aile üyelerini ekleyin.
                </p>
                {!isLocked && (
                  <Link
                    href="?add=1"
                    className="inline-flex items-center gap-2 rounded-xl bg-[#c7a76f] px-5 py-3 text-sm font-semibold text-[#091712] transition-colors hover:bg-[#d4b87c]"
                  >
                    + İlk üyeyi ekle
                  </Link>
                )}
                ...
                <div className="relative">
                  <FamilyTreeCanvas
                    ...
                    members={(members ?? []).map(m => ({
                      ...
                      parent_member_id:
                        (m as Record<string, unknown>).parent_member_id as string | null ?? null,
                    }))}
                  />
                </div>
              </div>
          ...
          {(members?.length ?? 0) > 0 && !isLocked && (
            <div className="mt-8">
              <div className="mb-4 flex items-center gap-3 text-[#c7a76f]">
                ...
                <span className="text-xs uppercase tracking-[0.2em]">Üye Yönetimi</span>
              </div>
              <div className="space-y-2">
                {(members ?? []).map(m => {
                  ...
                  const mYears = mBirthY
                    ? `${mBirthY}${!m.is_alive && mDeathY ? ` – ${mDeathY}` : m.is_alive ? ' –' : ''}`
                    : ''
                  return (
                    <div
                      key={m.id}
                      className="flex items-center gap-3 rounded-xl border border-white/8 bg-[#0d1412] px-4 py-3"
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="truncate font-serif text-sm text-white">
                          {m.full_name}
                        </div>
                      ...
                      <div className="flex shrink-0 items-center gap-3">
                        <Link
                          href={`?edit=${m.id}`}
                          className="text-xs font-semibold text-[#c7a76f] hover:underline"
                        >
                          Düzenle
                        </Link>
                        <form action={del}>
                          <button
                            type="submit"
                            className="text-xs font-semibold text-white/30 transition-colors hover:text-red-400"
                          >
                            Kaldır
                          </button>
                        </form>
