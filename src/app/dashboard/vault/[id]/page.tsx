...
export default async function MemoryAreaPage({ params, searchParams }: Props) {
  ...
  const checks = [
    ...
    { done: !!area.birth_date && !!area.death_date, weight: 10 },
    { done: !!area.tagline?.trim(), weight: 7 },
    { done: !!area.cover_photo_url, weight: 10 },
    { done: (photoCount ?? 0) > 0, weight: 13 },
    { done: (videoCount ?? 0) > 0, weight: 8 },
    { done: (memoriesCount ?? 0) > 0, weight: 10 },
    { done: (familyCount ?? 0) > 0, weight: 12 },
    { done: (documentCount ?? 0) > 0, weight: 9 },
  ...
  const sections: Section[] = [
    { href: 'biography',  label: 'Hayat Hikayesi',    meta: (area.biography?.length ?? 0) > 0 ? `${area.biography!.length} karakter` : 'Boş',               done: (area.biography?.length ?? 0) > 50, icon: BookOpen },
    ...
    { href: 'heirs',      label: 'Yetkili Kişiler',    meta: (heirCount ?? 0) > 0 ? `${heirCount} yetkili` : 'Eklenmedi',                                     done: (heirCount ?? 0) > 0, icon: Shield },
        ...
        <div className="mb-6 rounded-3xl border border-[#e5dccb] bg-[#fffdf8] p-5 shadow-[0_4px_24px_rgba(64,48,24,0.06)] sm:p-7">
          <div className="flex items-center gap-5">
            ...
            <div className="flex-1 min-w-0">
              ...
              {(birthYear || deathYear) && (
                <p className="font-serif text-sm text-[#788177] mb-1">
                  {birthYear ?? '?'} – {deathYear ?? '...'}
                </p>
              )}
              {area.tagline && (
