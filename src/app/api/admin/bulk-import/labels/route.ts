...
import { buildLabelsPdf, type LabelRecord } from '@/lib/labels/build-labels-pdf'
...
export async function GET(request: NextRequest) {
  ...
  const records: LabelRecord[] = vaults.map((v) => ({
    displayName: v.display_name,
    qrId: v.qr_id,
    loginUsername: v.login_username,
  }))

  const pdf = await buildLabelsPdf(records)

  if (batchId) {
    await supabase.from('vaults').update({ qr_label_printed: true }).eq('bulk_batch_id', batchId)
  } else {
    await supabase.from('vaults').update({ qr_label_printed: true }).eq('id', vaultId as string)
  }

  return new NextResponse(pdf as unknown as BodyInit, {
    headers: {
      'Content-Type': 'application/pdf',
      'Content-Disposition': `attachment; filename="etiketler-${batchId ?? vaultId}.pdf"`,
