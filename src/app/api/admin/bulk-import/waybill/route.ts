...
import { buildWaybillPdf, type WaybillRecord } from '@/lib/labels/build-waybill-pdf'
...
export async function GET(request: NextRequest) {
  ...
  let query = supabase
    .from('vaults')
    .select('id, display_name, shipping_address, phone, qr_id, bulk_batch_id')
  ...
  const records: WaybillRecord[] = vaults.map((v) => ({
  ...
  const pdf = await buildWaybillPdf(records)

  if (batchId) {
    await supabase
      .from('vaults')
      .update({ waybill_printed: true })
      .eq('bulk_batch_id', batchId)
  } else {
    await supabase.from('vaults').update({ waybill_printed: true }).eq('id', vaultId as string)
  }

  return new NextResponse(pdf as unknown as BodyInit, {
    headers: {
      'Content-Type': 'application/pdf',
      'Content-Disposition': `attachment; filename="irsaliye-${batchId ?? vaultId}.pdf"`,
